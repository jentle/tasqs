...
module.exports = class Publisher extends SQSClient
  ...
  publish:(taskClass, taskId, payload, retryNum=0, delaySec=0, cb) ->
    ...
    task_queue = queue_loader.getQueue taskClass::queue

    @_getQueue task_queue.name, (err, queue)->
      msg = QsMessage.create_message taskClass, taskId, payload, queue, retryNum
